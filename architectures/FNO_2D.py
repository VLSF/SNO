...
def random_i_layer_params(m, n, k, key):
    ...
    layer_parameters = (random.normal(w_key1, (2*k, k, n, m)) / m, random.normal(w_key2, (2*k, k, n, m)) / m, random.normal(s_key, (m, n)) / m, random.normal(b_key, (1, 1, n)))
    return layer_parameters
...
def NN_i(params, input, activation=utils.relu):
    n = len(params)
    for i, p in enumerate(params):
        w = jnp.roll(jnp.fft.rfftn(input, axes=(0, 1))[:, :p[0].shape[1]], p[0].shape[1], axis=0)[:p[0].shape[0]]
        w = dot_general(p[0] + 1j * p[1], w, (((3,), (2,)), ((0, 1), (0, 1))))
        w = jnp.roll(w, -p[0].shape[1], axis=0)
        input = jnp.dot(input, p[2]) + jnp.fft.irfftn(w, s=input.shape[:2], axes=(0, 1)) + p[3]
        if i < n-1:
            input = activation(input)
    return input
