...
def random_i_layer_params_r(m, n, c_m, c_n, key):
    keys = random.split(key, 3)
    layer_parameters = (random.normal(keys[0], (n, m)) / m, random.normal(keys[1], (c_m, c_n)) / c_m, random.normal(keys[2], (n, c_n)))
    return layer_parameters

def init_i_network_params_r(sizes, c_sizes, key):
    keys = random.split(key, len(sizes))
    return [random_i_layer_params_r(m, n, c_m, c_n, k) for m, n, c_m, c_n, k in zip(sizes[:-1], sizes[1:], c_sizes[:-1], c_sizes[1:], keys)]

def NN_i_r(params, input, activation=utils.softplus):
    n = len(params)
