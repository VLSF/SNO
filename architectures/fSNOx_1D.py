...
def random_c_layer_params(m, n, key):
    ...
    layer_parameters = [random.normal(key, (m, n)) / m]
    layer_parameters += [jnp.array([0.43353092]*n), jnp.array([0.04588344]*n), jnp.array([0.89730859]*n), jnp.array([0.32568061]*n), jnp.array([0.61873098]*n)]
    return tuple(layer_parameters)
...
def NN_c(params, input):
    n = len(params)
    for i, p in enumerate(params):
        input = jnp.dot(input, p[0])
        input = (input + p[1]) * (input + p[2]) * (input + p[3]) / ((input + p[4])**2 + p[5]**2)
    return input

def random_i_layer_params(m, n, c_m, c_n, key):
    keys = random.split(key, 6)
    layer_parameters = (random.normal(keys[0], (n, m)) / m, random.normal(keys[1], (n, m)) / m, random.normal(keys[2], (c_m, c_n)) / c_m, random.normal(keys[3], (c_m, c_n)) / c_m, random.normal(keys[4], (n, c_n)), random.normal(keys[5], (n, c_n)))
...
def NN_i(params, input, activation=utils.complex_split_softplus):
    n = len(params)
    for i, p in enumerate(params):
        input = jnp.dot(jnp.dot(p[0] + 1j * p[1], input), p[2] + 1j * p[3]) + p[4] + 1j * p[5]
        if i < n-1:
...
@partial(jit, static_argnums=2)
def NN(params, input):
    input = NN_c(params[0], input)
    K = input.shape[0]
    input_ = Fourier.values_to_coefficients(input)[:params[1][0][0].shape[1], :]
    input_ = NN_i(params[1], input_)
    input = Fourier.coefficients_to_values(input_, (K, 1)) + input
    input = NN_c(params[2], input)
    return input
...
def save_params(params, directory, filename):
    ...
    for i, p in enumerate(params[0]):
        ...
        for j in range(5):
            params_encoder_dict["b_" + str(j) + "_" + str(i)] = p[j+1]
    ...
    for i, p in enumerate(params[2]):
        ...
        for j in range(5):
            params_decoder_dict["b_" + str(j) + "_" + str(i)] = p[j+1]
...
def load_params(directory, filename):
    ...
    n = len(params_encoder_dict.files) // 6
    params_encoder = []
    for i in range(n):
        params_encoder.append(tuple([params_encoder_dict["w_" + str(i)]] + [params_encoder_dict["b_" + str(j) + "_" + str(i)] for j in range(5)]))

    params_i_dict = jnp.load(directory + "/" + "i_" + filename)
    n = len(params_i_dict.files) // 6
    params_i = []
    for i in range(n):
        params_i.append((params_i_dict["w_r_" + str(i)], params_i_dict["w_c_" + str(i)], params_i_dict["s_r_" + str(i)], params_i_dict["s_c_" + str(i)], params_i_dict["b_r_" + str(i)], params_i_dict["b_c_" + str(i)]))

    params_decoder_dict = jnp.load(directory + "/" + "decoder_" + filename)
    n = len(params_decoder_dict.files) // 6
    params_decoder = []
    for i in range(n):
        params_decoder.append(tuple([params_decoder_dict["w_" + str(i)]] + [params_encoder_dict["b_" + str(j) + "_" + str(i)] for j in range(5)]))
